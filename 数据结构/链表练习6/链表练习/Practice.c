...
void FindCrossNode(SList *list1, SList *list2)
{
	assert(list1!= NULL);
	assert(list2!= NULL);
	if (list1->first == NULL || list2->first == NULL)
...
int IsCircle(SList *list)
{
	...
	SListNode *first = list->first;
	SListNode *slow = list->first;
...
SListNode *FindEnterNode(SList *list)
{
	SListNode *first = list->first;
	SListNode *slow = list->first;
	SListNode *cur = list->first;
...
void TestIsCircleAndEnterNode()
{
	...
	SListNode *back = SListTail(&list);
	back->next = SListFind(&list, 4);
	int ret = IsCircle(&list);
	if (ret == 1)
	{
		printf("链表带环\n");
	}
	SListNode *cur = FindEnterNode(&list);
	printf("入口点的值为%d\n", cur->value);
	int len = CircleLen(&list);
	printf("环的长度为%d\n", len);
}